...
void telemetry_data(void* self) {
  ...
  fprintf(file, "%u, ", state->nominal.setpoint_ack_rise);
  ...
  fprintf(file, "%u, ", private_state->rxpdo_common.controlword);
  fprintf(file, "%u, ",
          ((private_state->rxpdo_common.controlword >> 4) & 0x01));
  fprintf(file, "%u, ",
          ((private_state->rxpdo_common.controlword >> 5) & 0x01));
  fprintf(file, "%u, ",
          ((private_state->rxpdo_common.controlword >> 6) & 0x01));
...
void command(void* self) {
  ...
  // command.
  if (state->nominal.setpoint_ack_rise) {
