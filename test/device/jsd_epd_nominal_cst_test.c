...
void command(void* self) {
  ...
  jsd_epd_nominal_set_motion_command_cst(sds->jsd, slave_id, cst);
  ++iter;
}
...
int main(int argc, char* argv[]) {
  if (argc != 7) {
    ...
    MSG("Usage: jsd_epd_nominal_cst_test <ifname> <epd_slave_index> "
        "<loop_freq_hz> "
        "<target_current> <peak_current_amps> <continuous_current_amps> ");
    MSG("Example: $ jsd_epd_nominal_cst_test eth0 2 100 0.1 1.0 0.45");
    return 0;
  }
  ...
  config.epd_nominal.max_motor_speed          = 1e9;
  ...
  config.epd_nominal.over_speed_threshold           = 0.0;
  ...
  sds_run(&sds, ifname, "/tmp/jsd_epd_nominal_cst_test.csv");
