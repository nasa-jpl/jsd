...
void command(void* self) {
  ...
  jsd_epd_nominal_set_motion_command_csv(sds->jsd, slave_id, csv);
}

int main(int argc, char* argv[]) {
  if (argc != 10) {
    ERROR("Expecting exactly 9 arguments");
    MSG("Usage: jsd_epd_nominal_csv_test <ifname> <epd_slave_index> "
        "<loop_freq_hz> "
        "<amplitude> <sine_freq> <over_speed_threshold> <peak_current_amps> "
        "<continuous_current_amps> <max_motor_speed> ");
    MSG("Example: $ jsd_epd_nominal_csv_test eth0 2 100 25000 35000 1.0 0.45 "
        "50000");
    return 0;
  }
  ...
  config.epd_nominal.over_speed_threshold           = over_speed_threshold;
  ...
  sds_run(&sds, ifname, "/tmp/jsd_epd_nominal_csv_sine_test.csv");
