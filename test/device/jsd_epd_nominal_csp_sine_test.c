...
void command(void* self) {
  ...
  jsd_epd_nominal_set_motion_command_csp(sds->jsd, slave_id, csp);
}
...
int main(int argc, char* argv[]) {
  if (argc != 10) {
    ...
    MSG("Example: $ jsd_epd_nominal_csp_sine_test eth0 2 100 25000 0.25 1 0.25 "
        "0.45 "
        "50000");
    return 0;
  }
  ...
  sds_run(&sds, ifname, "/tmp/jsd_epd_nominal_csp_sine_test.csv");
