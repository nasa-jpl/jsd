...
static bool jsd_ati_fts_init(jsd_t* self, uint16_t slave_id) {
  assert(self);
  assert(self->ecx_context.slavelist[slave_id].eep_id ==
         JSD_ATI_FTS_PRODUCT_CODE);
...
static int jsd_ati_fts_PO2SO_config(ecx_contextt* ecx_context,
                                    uint16_t      slave_id) {
  assert(ecx_context);
...
static bool jsd_ati_fts_parse_status_code(uint32_t status_code,
                                          uint16_t slave_id) {
...
static const char* jsd_ati_fts_force_unit_to_string(uint8_t force_units) {
  switch (force_units) {
    case 1:
      return "Lbf";
      break;
...
static const char* jsd_ati_fts_torque_unit_to_string(uint8_t torque_units) {
  switch (torque_units) {
    case 1:
      return "Lbf-in";
      break;
