...
typedef enum {
  ...
  JSD_EL3202_CONNECTION_4WIRE,          // This should be default and other wiring options will fault. To use 2 or 3 wire, use jumper cable
...
typedef enum {
  ...
  JSD_EL3202_PRESENTATION_HIGH_RES = 2,  ///< For EL320x-00x0 devices only, should be used with PT100 only
...
typedef struct {
  ...
  jsd_el3202_presentation_t
      presentation[JSD_EL3202_NUM_CHANNELS];  ///< Data presentation, default: low rez(1/10 C)
} jsd_el3202_config_t;
...
typedef struct {
  ...
  int16_t adc_value[JSD_EL3202_NUM_CHANNELS];      ///< sensed digital converted analog value
