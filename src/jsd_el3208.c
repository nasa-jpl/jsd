...
static bool jsd_el3208_init(jsd_t* self, uint16_t slave_id) {
  assert(self);
  assert(self->ecx_context.slavelist[slave_id].eep_id ==
         JSD_EL3208_PRODUCT_CODE);
...
static int jsd_el3208_PO2SO_config(ecx_contextt* ecx_context,
                                   uint16_t      slave_id) {
  assert(ecx_context);
...
static double jsd_el3208_output_from_config(int16_t              adc_value,
