...
typedef struct __attribute__((__packed__)) {
  uint16_t flags;
  int16_t  value;
} jsd_el3318_channel_txpdo_t;

/**
 * @brief Private IOmap struct used to retreive input data from SOEM IOmap
 */
typedef struct __attribute__((__packed__)) {
  jsd_el3318_channel_txpdo_t channel[JSD_EL3318_NUM_CHANNELS];
} jsd_el3318_txpdo_t;

/** @brief Initializes el3318 and registers the PO2SO function
 *
 ...
 * transition that (re)configures el3318 device settings
 *
...
/**
 * @brief Checks whether a product code is compatible with EL3318.
 *
 * @param product_code The product code to be checked
 * @return True if the product code is compatible, false otherwise.
 */
bool jsd_el3318_product_code_is_compatible(uint32_t product_code);
