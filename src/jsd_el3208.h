...
typedef struct __attribute__((__packed__)) {
  uint16_t flags;
  int16_t  value;
} jsd_el3208_channel_txpdo_t;

/**
 * @brief Private IOmap struct used to retreive input data from SOEM IOmap
 */
typedef struct __attribute__((__packed__)) {
  jsd_el3208_channel_txpdo_t channel[JSD_EL3208_NUM_CHANNELS];
} jsd_el3208_txpdo_t;

/** @brief Initializes el3208 and registers the PO2SO function
 *
 ...
 * transition that (re)configures el3208 device settings
 *
